  ...
  return <main className="relative min-h-screen overflow-hidden px-5 py-10">
    ...
    <form onSubmit={submit} className="space-y-5">
      <div className="grid gap-5 sm:grid-cols-2">
        <Field label="Full name" id="register-name"><input id="register-name" className="input-field bg-white/80" name="name" autoComplete="name" value={form.name} onChange={change} placeholder="Your name" required/></Field>
        <Field label="Email address" id="register-email"><input id="register-email" className="input-field bg-white/80" type="email" name="email" autoComplete="email" value={form.email} onChange={change} placeholder="you@example.com" required/></Field>
      </div>
      <Field label="Password" id="register-password"><div className="relative"><input id="register-password" className="input-field bg-white/80 pr-12" type={show?"text":"password"} name="password" autoComplete="new-password" value={form.password} onChange={change} placeholder="Choose any password" required/><button type="button" onClick={()=>setShow(!show)} aria-label={show?"Hide password":"Show password"} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-slate-400">{show?<EyeOff size={18}/>:<Eye size={18}/>}</button></div><p className="mt-2 text-xs text-slate-500">No password complexity rules are applied.</p></Field>
      <div className="grid gap-5 sm:grid-cols-2"><Field label="University" id="register-university"><input id="register-university" className="input-field bg-white/80" name="university" autoComplete="organization" value={form.university} onChange={change} placeholder="University name"/></Field><Field label="Course" id="register-course"><input id="register-course" className="input-field bg-white/80" name="course" autoComplete="off" value={form.course} onChange={change} placeholder="B.Tech"/></Field></div>
      <div className="grid gap-5 sm:grid-cols-2"><Field label="Branch" id="register-branch"><input id="register-branch" className="input-field bg-white/80" name="branch" autoComplete="off" value={form.branch} onChange={change} placeholder="Computer Science"/></Field><Field label="Current semester" id="register-semester"><select id="register-semester" className="input-field bg-white/80" name="semester" autoComplete="off" value={form.semester} onChange={change}>{Array.from({length:12},(_,i)=><option key={i+1}>{i+1}</option>)}</select></Field></div>
...
function Field({label,id,children}){return <div><label htmlFor={id} className="mb-2 block text-sm font-semibold text-slate-700">{label}</label>{children}</div>}